...
# Step 8: Filter for Myeloid Cells
cat("Filtering for myeloid cells...\n")
myeloid_barcodes <- JY10_annotations %>%
  filter(`Graph-based` == "Myeloid_cells") %>%
  select(Barcode)

# Standardize barcodes
myeloid_barcodes$Barcode <- standardize_barcodes(myeloid_barcodes$Barcode)

# Step 9: Merge Myeloid Cells with Spatial Data and Tumor Labels
cat("Merging myeloid cells with spatial data...\n")
...
myeloid_spatial_data$Label[is.na(myeloid_spatial_data$Label)] <- "Non-Tumor"

# Step 10: Separate Myeloid Cells Inside and Outside Tumor Boundary
cat("Separating myeloid cells based on tumor boundary...\n")
...
cat("Number of myeloid cells inside tumor boundary:", nrow(myeloid_inside_boundary), "\n")
cat("Number of myeloid cells outside tumor boundary:", nrow(myeloid_outside_boundary), "\n")

# Step 11: Extract Counts Data for Myeloid Cells Only
cat("Extracting counts data for myeloid cells...\n")
...
# Filter counts_data to include only myeloid cells
myeloid_cells <- myeloid_barcodes$Barcode
counts_data_myeloid <- counts_data[, colnames(counts_data) %in% myeloid_cells]

# Confirm dimensions
cat("Counts data dimensions for myeloid cells:", dim(counts_data_myeloid), "\n")

# Step 12: Identify Inside and Outside Myeloid Cells in Counts Data
cat("Identifying inside and outside myeloid cells in counts data...\n")
inside_cells <- intersect(myeloid_inside_boundary$cell, colnames(counts_data_myeloid))
outside_cells <- intersect(myeloid_outside_boundary$cell, colnames(counts_data_myeloid))

# Confirm cell counts
cat("Number of inside myeloid cells:", length(inside_cells), "\n")
cat("Number of outside myeloid cells:", length(outside_cells), "\n")

# Step 13: Create Seurat Object with Myeloid Cells Only
cat("Creating Seurat object with myeloid cells only...\n")
seurat_obj <- CreateSeuratObject(counts = counts_data_myeloid)
...
cat("Assigning group labels to myeloid cells...\n")
...
cat("Significant genes saved to 'significant_genes.csv'.\n")

# Optional: Visualize top significant genes
cat("Visualizing top significant genes...\n")
top_genes <- head(rownames(significant_genes), 20)
VlnPlot(seurat_obj, features = top_genes, group.by = "group")
...
# Save the heatmap
png("heatmap_top_genes.png", width = 800, height = 600)
print(heatmap_plot)
dev.off()
...
ggsave("violin_plots_top_genes.png", plot = violin_plot, width = 12, height = 8)
cat("Violin plots saved as 'violin_plots_top_genes.png'.\n")

# Display the plot
print(violin_plot)

cat("Script execution completed with enhanced visualizations.\n")
